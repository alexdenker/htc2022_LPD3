...
def main(inputFolder, outputFolder, categoryNbr):
    ...
    for f in os.listdir(inputFolder):
        ...
        with torch.no_grad():
            x_hat = torch.round(torch.sigmoid(model(y)))
            ...
            for phi in np.linspace(0, 180, 50):
                ...
                y_in = ray_trafo_torch(x_rotate)

                x_rot = torch.sigmoid(model(y_in))
                filter = threshold_otsu(x_rot.cpu().numpy()[0,:,:,:])

                x_seg = rotate(x_rot, -phi).cpu().numpy()[0,:,:,:]
                x_seg[x_seg < filter] = 0.
                x_seg[x_seg >= filter] = 1.

                xs.append(x_seg)

        x_seg = np.round(np.mean(np.asarray(xs), axis=0))[0,:,:]
